...
<section id="section-{sec_idx}" class="page ag-page" style="border-top:2px solid var(--border)" data-agency="{a["name"]}">
  <div class="ag-head" style="background:{bg};border-left:4px solid {bd}">
    ...
    <div class="ag-kpis">
      ...
      <button class="section-export-btn" onclick="exportSection('section-{sec_idx}')">⬇ PDF</button>
...
.section-export-btn {{
  ...
  margin-left: 12px; vertical-align: middle; opacity: .85;
}}
...
<section id="section-1" class="page" style="border-top:2px solid var(--border)">
  <div class="sec-label">02 — Competitiveness in Pitches  <button class="section-export-btn" onclick="exportSection('section-1')">⬇ PDF Rankings</button></div>
...
window.exportSection = function(sectionId) {{
  ...
  document.querySelectorAll('.section-export-btn, .edit-del-btn, .add-row-btn, .edit-color-btn, #edit-toolbar').forEach(function(b) {{ b.style.visibility = 'hidden'; }});
  window.print();
  setTimeout(function() {{
    saved.forEach(function(item) {{ item[0].style.display = item[1]; }});
    document.querySelectorAll('.section-export-btn, .edit-del-btn, .add-row-btn, .edit-color-btn, #edit-toolbar').forEach(function(b) {{ b.style.visibility = ''; }});
  }}, 900);
}};
</script>
</body>
