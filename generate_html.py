...
def build_agencies_overview(data, threshold=5.0):
    ...
    for a in agencies:
        ...
        chart_bars += f'''<div class="bar-row" data-agency="{a["agency"]}">
          <div class="bar-label" style="background:{bg};border-left:3px solid {border};padding:2px 8px;border-radius:4px;font-weight:700;white-space:nowrap">{a["agency"].upper()}</div>
          <div class="bar-track">
  ...
  <div class="page-header">
    <div class="section-header-inner"><span class="section-num">03</span><h2>NBB 2025 agencies overview <span class="year">· Sep.24 – Sep.25</span></h2><button class="section-export-btn" onclick="exportSection('section-2')">⬇ PDF</button></div>
...
def build_retentions(data):
    ...
    for r in ret_data[:8]:
        ...
        is_neg = r['balance'] < 0
        # Always green gradient for retentions (positive = kept); red if rare negative
        gradient = (
            "linear-gradient(90deg, #86EFAC 0%, #1A6B4A 100%)" if not is_neg
            else "linear-gradient(90deg, #C0392B 0%, #FCA5A5 100%)"
        )
        ag_name = r["agency"].upper()
        bars += f'''<div class="ret-bar-row">
          <div class="ret-bar-label" style="background:{bg};border-left:4px solid {border};font-weight:700;padding:3px 8px;border-radius:4px;white-space:nowrap">
            {ag_name}
          </div>
          <div class="ret-bar-track">
            <div class="ret-bar-fill" style="width:{pct:.1f}%;background:{gradient}"></div>
          </div>
          <div class="ret-val" style="color:{'#1A6B4A' if not is_neg else '#C0392B'}">{fmt(r["balance"])}</div>
        </div>'''
    ...
    for r in ret_data[:8]:
        ...
        border = group_border(grp)
        is_neg = r['balance'] < 0
        val_color = '#1A6B4A' if not is_neg else '#C0392B'
        ag_badge = f'<span style="background:{bg};border-left:3px solid {border};padding:2px 8px;border-radius:4px;font-weight:700;font-size:12px">{r["agency"].upper()}</span>'
        table_rows += f'''<tr>
          <td style="padding:6px 10px">{ag_badge}</td>
          <td class="td-nbb" style="color:{val_color};font-weight:700;font-family:var(--mono)">{fmt(r["balance"])}</td>
          <td class="td-topclient">{r["top_client"]}</td>
        </tr>'''

    return f'''<section id="section-4" class="page">
  <div class="page-header">
    <div class="section-header-inner"><span class="section-num">05</span><h2>NBB 2025 retentions ranking <span class="year">· Sep.24 – Sep.25</span></h2><button class="section-export-btn" onclick="exportSection('section-4')">⬇ PDF</button></div>
...
.kf-table tbody tr { transition: background .15s ease; }
.kf-table tbody tr:hover { background: rgba(56,189,248,.06) !important; box-shadow: inset 3px 0 0 #38BDF8; }
.bar-row { transition: background .15s ease, padding-left .1s ease; border-radius: 6px; }
.bar-row:hover { background: rgba(56,189,248,.06) !important; padding-left: 8px; }
.ret-bar-row { transition: background .15s ease, transform .1s ease; border-radius: 6px; }
.ret-bar-row:hover { background: rgba(56,189,248,.06) !important; transform: translateX(3px); }
.kf-row { transition: background .15s ease, box-shadow .15s ease; }
.kf-row:hover { background: rgba(56,189,248,.05) !important; box-shadow: inset 3px 0 0 #38BDF8; }
.agency-card { transition: box-shadow .2s ease, transform .15s ease; }
.agency-card:hover { box-shadow: 0 6px 24px rgba(0,0,0,.12) !important; transform: translateY(-2px); }
/* Per-section PDF export button */
.section-export-btn {
  ...
  transition: background .2s; margin-left: 12px; vertical-align: middle; opacity: .85;
}
.section-export-btn:hover { background: #7C3AED; opacity: 1; }
@media print { .section-export-btn { display: none !important; } }
...
window.exportSection = function(sectionId) {
  ...
  document.querySelectorAll('body > *').forEach(function(el) {
    if (!el.contains(sec) && el !== sec && el.tagName !== 'STYLE' && el.tagName !== 'SCRIPT') {
      saved.push([el, el.style.display]);
      el.style.display = 'none';
    }
  });
  document.querySelectorAll('.section-export-btn').forEach(function(b) { b.style.visibility = 'hidden'; });
  window.print();
  setTimeout(function() {
    saved.forEach(function(item) { item[0].style.display = item[1]; });
    document.querySelectorAll('.section-export-btn').forEach(function(b) { b.style.visibility = ''; });
  }, 900);
};
