...
<style>
  ...
  *,*::before,*::after{box-sizing:border-box;margin:0;padding:0;}
  ...
  header{width:100%;padding:16px 40px;border-bottom:1px solid var(--border);
  ...
  main{width:100%;max-width:620px;padding:50px 24px;display:flex;flex-direction:column;gap:24px;}
  .tagline{text-align:center;}
  .tagline h2{font-size:26px;font-weight:300;color:#fff;letter-spacing:-.02em;}
  ...
  .dropzone{border:1.5px dashed var(--border);border-radius:8px;padding:28px;
  ...
  .fmt-btn{padding:12px;border-radius:8px;border:1.5px solid var(--border);
  ...
  .dl-btn{display:flex;align-items:center;gap:8px;padding:10px 16px;border-radius:6px;
          text-decoration:none;font-weight:600;font-size:13px;transition:opacity .2s;}
  ...
  .col-row{display:flex;align-items:center;gap:10px;padding:7px 10px;
...
<main>
  <div class="tagline">
    ...
    <p>Génère automatiquement un rapport PPTX et un rapport HTML responsive.</p>
  </div>
  ...
  <div class="card">
    ...
    <div class="format-choice">
      ...
      <button class="fmt-btn" id="btnHtml" disabled>
        <span class="fmt-icon">🌐</span>
        <strong>HTML</strong>
        <span class="fmt-label">Rapport web responsive</span>
      </button>
    </div>
...
const btnPptx=document.getElementById('btnPptx'),btnHtml=document.getElementById('btnHtml');
...
function onFile(f){
  ...
  btnPptx.disabled=false; btnHtml.disabled=false;
}

async function generate(format){
  if(!fi.files[0])return;
  btnPptx.disabled=true; btnHtml.disabled=true;
  st.className='status loading';
  st.innerHTML='<div class="spinner"></div><span>Génération en cours…</span>';
  const fd=new FormData(); fd.append('file',fi.files[0]); fd.append('format',format);
  try{
    const res=await fetch('/generate',{method:'POST',body:fd});
    const d=await res.json();
    if(d.status==='success'){
      st.className='status';
      const cls=format==='pptx'?'dl-pptx':'dl-html';
      const icon=format==='pptx'?'📑':'🌐';
      st.innerHTML=`<div class="results">
        <a class="dl-btn ${cls}" href="${d.download_url}" target="_blank">${icon} Télécharger ${d.filename}</a>
      </div>`;
    }else{
      st.className='status';
...
btnPptx.addEventListener('click',()=>generate('pptx'));
btnHtml.addEventListener('click',()=>generate('html'));
</script>
</body>
</html>"""
